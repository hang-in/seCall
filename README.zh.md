...
- **向量语义搜索**: [Ollama](https://ollama.com/) `qwen3-embedding:0.6b` 嵌入（默认，1024 维；ONNX/ort 路径使用 BAAI/bge-m3，同为 1024 维）+ **HNSW ANN 索引**（[usearch](https://github.com/unum-cloud/usearch)），实现 O(log n) 检索
...
| 嵌入          | Ollama `qwen3-embedding:0.6b`（默认，1024 维）/ ONNX Runtime BAAI/bge-m3（可选，1024 维） |
