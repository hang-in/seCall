...
§1~8 이 "문서를 어디에 두고 어떻게 상태 관리하나" 라면, 여기는 "본문을 어떻게 쓰나" 다.
사람이 읽는 GitHub 렌더링과 에이전트 파싱을 모두 안정적으로 만들기 위한 포맷 규칙이며, 모든 문서에 권장한다.
이 중 공개 문서(**4개 README · `CLAUDE.md`**)는 `.markdownlint-cli2.jsonc` + GitHub Actions 로 자동 검사한다.
(`docs/reference/` 등 내부 WIP 문서는 CI 강제 대신 가이드로 준수한다.)

### 9.1 제목 (heading)

- ATX 스타일(`#`)만 사용한다. Setext(밑줄 `===`/`---`)는 금지.
- 문서당 최상위 `#` 는 하나(문서 제목), 섹션은 `##` 부터 시작한다.
- 헤딩 위아래로 빈 줄 하나를 둔다. 레벨을 건너뛰지 않는다(`##` → `####` 금지).

### 9.2 목록

- 순서 없는 목록은 `-` 로 통일한다(`*`/`+` 금지).
- 순서 있는 목록은 `1.` 로 시작한다.
- 중첩은 스페이스 2칸(부모 마커 기준) 들여쓰기.

### 9.3 코드블록

- fenced code(```)를 쓰고 **언어를 반드시 지정**한다: `bash` `rust` `json` `toml` `text` 등.
- 셸 예시는 `bash`, 출력·트리·순수 텍스트는 `text` 로 명시한다.
- 파일 경로/명령/식별자 등 짧은 인라인은 백틱 하나.

### 9.4 표

- GitHub 파이프 표를 쓰고 **헤더 구분행**(`| --- |`)을 반드시 둔다.
- 정렬 콜론(`:---`/`---:`)은 필요할 때만. CJK 폭 파이프 정렬은 강제하지 않는다(`MD060` off).

### 9.5 링크

- 같은 레포 문서는 **상대경로**로 링크한다(`../reference/api.md`). 절대 파일 경로(`/Users/...`) 링크 금지.
- 외부 링크는 전체 URL. 배지 URL의 `&` 는 **이스케이프하지 않는다**(`\&` 금지 — 4개 README 통일 기준).
- 소스 코드 위치를 가리킬 때는 클릭 링크가 아니라 인용 표기 `path/to/file.rs:239` 또는 `path#L239` 를 쓴다(줄번호가 붙은 경로를 `[]()` 링크로 만들지 않는다 — 링크 검사에서 깨진 링크로 잡힌다).

### 9.6 Admonition / 인용

- 강조 노트는 GitHub alert 문법 `> [!NOTE]` `> [!WARNING]` `> [!IMPORTANT]` 을 허용하되 남용하지 않는다.
- 일반 인용/보충은 blockquote `>`.

### 9.7 공백·개행

- 줄바꿈은 원문 유지(prettier `--prose-wrap preserve`). 줄 길이는 강제하지 않는다(`MD013` off).
- 인라인 HTML(`<div align="center">` 등)은 README 배지/가운데 정렬에 한해 허용한다.
- 파일 끝에 개행 하나. 문단·헤딩·리스트·코드블록 사이 빈 줄로 구분한다.
