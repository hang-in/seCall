...
task_number: 2
title: "Vault Git 연동 (init/pull/push)"
parallel_group: A
depends_on: []
updated_at: 2026-04-06
---

# Task 02: Vault Git 연동 (init/pull/push)
...
| 파일 | 변경 | 비고 |
|---|---|---|
| `crates/secall-core/src/vault/config.rs` | 수정 | `VaultConfig` (line 16-19)에 `git_remote: Option<String>` 추가 |
| `crates/secall-core/src/vault/git.rs` | 신규 | `VaultGit` struct — git 명령 래퍼 (init/pull/push/status) |
| `crates/secall-core/src/vault/mod.rs` | 수정 | `pub mod git;` 추가 (현재 exports: lines 5-11) |
| `crates/secall/src/commands/init.rs` | 수정 | `--git <remote>` 옵션 추가 (현재 run(): lines 9-58) |
| `crates/secall/src/main.rs` | 수정 | Init 서브커맨드에 `--git` arg 추가 (현재 Init: lines 23-27) |

## Change description

### Step 1: VaultConfig에 git 설정 추가

`crates/secall-core/src/vault/config.rs` — VaultConfig (lines 16-19):
...
impl<'a> VaultGit<'a> {
    ...
    fn run_git(&self, args: &[&str]) -> crate::error::Result<std::process::Output> {
        ...
        if !output.status.success() {
            ...
            return Err(crate::SecallError::Config(
...
`crates/secall/src/main.rs` — Init 서브커맨드 (lines 23-27)에 arg 추가:
...
`crates/secall/src/commands/init.rs` — run() (lines 9-58) 끝에 git 초기화 로직 추가:
...
### Step 4: vault/mod.rs에 모듈 등록

`crates/secall-core/src/vault/mod.rs` (lines 5-11)에 추가:
...
- 없음 (git은 시스템 명령 호출, 별도 crate 불필요)
...
# 5. git 미설치 환경에서 graceful failure 확인
# Manual: PATH에서 git 제거 후 secall init --git 실행 → 에러 메시지 확인
```

## Risks

- **git 미설치**: 시스템에 git이 없으면 실패. `which git` 사전 체크 후 명확한 에러 메시지 출력 고려.
- **인증 문제**: SSH key 미설정 시 push/pull 실패. 사용자 책임이나, 에러 메시지에 안내 포함.
- **branch 전략**: 1차에서는 `main` 단일 브랜치. 세션이 충돌하지 않으므로 멀티 브랜치 불필요.
- **대규모 vault**: 수천 개 MD 파일의 git push가 느릴 수 있음. shallow clone은 1차에서 미구현.
...
- `crates/secall-core/src/search/bm25.rs` — Task 04 영역
