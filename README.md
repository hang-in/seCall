...
seCall은 Karpathy의 LLM Wiki 아이디어를 AI 에이전트 작업 기록에 적용한 로컬 지식베이스 도구입니다.

Claude Code, Codex CLI, Gemini CLI, claude.ai, ChatGPT 대화를 수집해 원본 transcript는 그대로 보존하고, 그 위에 AI가 정리한 프로젝트·토픽·의사결정 위키를 생성합니다.

즉, 흩어진 AI 대화를 단순 로그가 아니라 계속 갱신되는 개인 작업 위키로 바꿉니다.

검색은 SQLite FTS5 기반 BM25, BGE-M3 벡터 검색, RRF 결합을 사용하며 CLI, Web UI, Obsidian, MCP 서버에서 모두 접근할 수 있습니다.

DB는 파생 캐시이고, 정본은 Obsidian 호환 Markdown 볼트입니다. Git 동기화를 통해 여러 기기에서 같은 위키를 사용할 수 있습니다.

AI 에이전트와 나눈 대화를 로컬 위키로 정리하고 검색하는 도구입니다.

Claude Code, Codex CLI, Gemini CLI, claude.ai, ChatGPT 세션을 수집해 Obsidian 호환 Markdown 볼트로 저장합니다. 저장된 기록은 BM25 검색, 벡터 검색, MCP 서버, Web UI, Obsidian 플러그인을 통해 다시 찾아볼 수 있습니다.

<div align="center">""Rust" (https://img.shields.io/badge/Rust-1.75+-f74c00?logo=rust&logoColor=white)" (https://www.rust-lang.org/)
...
AI 에이전트를 오래 사용하면 중요한 작업 기록이 여러 위치에 흩어집니다.

- Claude Code / Codex CLI / Gemini CLI 세션 로그
...
Linux는 아직 prebuilt 바이너리를 제공하지 않습니다. Cargo 빌드를 사용하세요.
...
«"cargo install secall"은 웹 UI 빌드를 자동으로 수행하지 않습니다. Web UI가 필요하면 Release 바이너리 또는 "just build"를 사용하세요.»
...
검색, 세션 메타데이터, 명령 실행, Job, SSE 등 REST API 엔드포인트 전체는 "API 레퍼런스" (docs/reference/api.md) 를 참고하세요.
...
"recall"| 과거 세션 검색
"get"| 특정 세션 조회
"status"| 인덱스와 설정 상태 확인
"wiki_search"| 생성된 위키 검색
"graph_query"| Knowledge Graph 질의

MCP 클라이언트 설정과 세션 hooks 자동 동기화 예시는 "API 레퍼런스" (docs/reference/api.md) 를 참고하세요.
...
seCall은 세션과 위키에서 관계를 추출해 그래프를 만듭니다.
...
관련 프로젝트
참고한 프로젝트는 아니지만, 같은 패턴을 독립적으로 구현한 이웃 작업입니다.

- "nashsu/llm_wiki" (https://github.com/nashsu/llm_wiki) — LLM Wiki 패턴을 독립 구현한 풀 데스크톱 앱입니다. source 추적, 지식그래프, 시맨틱 검색, MCP, Obsidian 호환 기능을 제공합니다. seCall과 소스 관계는 없으며, 동일한 패턴을 다른 방식으로 적용한 사례입니다. seCall은 세션 로그 중심 CLI/MCP 도구이고, "llm_wiki"는 데스크톱 앱에 가깝습니다.
...
전체 버전과 Phase 변경 이력은 "CHANGELOG.md" (CHANGELOG.md) 에서 관리합니다.
...
<div align="center">Contact: "d9ng@outlook.com" (mailto:d9ng@outlook.com)
